...
void basic_test(size_t N, size_t logq) {
        ...
        for (size_t i = 0; i < TEST_NUMBER_ITERS; i++) {
            ...
            NTL::ZZ x_reconstructed = rns.from_rns_classic(rns_rep);
            EXPECT_EQ(x_reconstructed, x % rns.get_M()); // Ensure reconstruction is correct modulo M
        }
}


void classic_recover_test(size_t N, size_t logq) {
    uint64_t moduli[N];
...
void small_recover_test(size_t N, size_t logq) {
    uint64_t moduli[N];
    ...
    for (size_t i = 0; i < TEST_NUMBER_ITERS; i++) {
        ...
        NTL::ZZ x_reconstructed = rns.from_rns_small(rns_rep);
        EXPECT_EQ(x_reconstructed, x % rns.get_M()); // Ensure reconstruction is correct modulo M
    }
}

void approx_recover_test(size_t N, size_t logq) {
    uint64_t moduli[N];
...
TEST(RNS, ClassicRecoverTest) {
    for (size_t j = 0; j < TEST_RNS_ITERS; j++) {
    
        size_t N = 4 + (rand() % 21);
        size_t logq = 20 + (rand() % 35);

        classic_recover_test(N, logq);
    }
}

TEST(RNS, SmallRecoverTest) {
    for (size_t j = 0; j < TEST_RNS_ITERS; j++) {
    
        size_t N = 4 + (rand() % 21);
        size_t logq = 20 + (rand() % 35);

        small_recover_test(N, logq);
    }
}

TEST(RNS, ApproxRecoverTest) {
    for (size_t j = 0; j < TEST_RNS_ITERS; j++) {
